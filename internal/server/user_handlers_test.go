...
func TestPostAPIUserRegister(t *testing.T) {
	t.Run("empty login returns 400", func(t *testing.T) {
		...
		srv := NewServer(mocks.NewMockAuth(ctrl), mocks.NewMockUserService(ctrl), mocks.NewMockOrderService(ctrl))
	...
	t.Run("hash password error returns error", func(t *testing.T) {
		...
		srv := NewServer(mockAuth, mocks.NewMockUserService(ctrl), mocks.NewMockOrderService(ctrl))
	...
	t.Run("user already exists returns 409", func(t *testing.T) {
		...
		srv := NewServer(mockAuth, mockUsers, mocks.NewMockOrderService(ctrl))
